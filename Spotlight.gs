function YoutubeScrape() {
  ...
  var headers = ['Img', 'Order', 'Title', 'Type', 'Time', 'Quality', 'Date', 'Description'];
  ...
  $('div.deslide-item').each(function (index, element) {
    ...
    var order = $(element).find('.desi-sub-text').text();
    ...
    sheet.appendRow([imgSrc, order, title, type, time, quality, date, description]);
  });
}
